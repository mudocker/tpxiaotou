...
class ArticleController extends AdminController {
	...
	/**生成文章html**/
	public function createArcHtml($id){
		$id = $id?$id:I('id');
		if(empty($id)){
			return;
		}
		...
		//文件夹路径
		$filepath =$this->getCatepath($info['category_id']);
		if(!is_dir($filepath)){
			mkdir($filepath,0777,true);		
		}
		$this->assign('info',$info);
		$tempath = 'Article/detail';
		if(!file_exists('./template/Home/classic/'.$tempath.'.html')) $this->error('模板不存在，没有成功生成文章');
		$this->buildHtml($id,HTML_PATH.$filepath.'/',C('HOME_TEMPLATE').$tempath);
	}
	...
	public function test(){
		//如果id为空，就获取第一个id
		if(empty($_GET['id'])){
			$tmp = M('Document')->field('id')->order('id asc')->limit('1')->select();
			//dump($tmp);
			$_GET['id'] = $tmp[0]['id'];
		}
		//获取文章ID并读取文章
		$info = M("Document")->where('id='.$_GET['id'])->select();

		$this->assign('info',$info[0]);
		$this->display('Home@default:Article:test');
		$this->buildHtml('1',HTML_PATH.'/','default/Article/test','utf-8');
	}


	//文章静态化
	public function htmlForArt(){ 
		//如果id为空，就获取第一个id
		if(empty($_GET['id'])){
			$tmp = M('Document')->field('id')->order('id asc')->limit('1')->select();
			//dump($tmp);
...
//		$comment = D('comment')->field("adduser,content,addtime")->where('cid='.$_GET['id'].' and isable=1')->order(' addtime desc')->select();
		...
		if(empty($info)){
			...
			foreach($info as $v){
				$articleInfo = array('title'=>$v['title'],'id'=>$_GET['id'],'content'=>htmlspecialchars_decode($v['content']),'author'=>$v['author'],'addtime'=>$v['addtime'],'artfrom'=>$v['artfrom']);
			}
			//文章点击次数
			//$_POST['clicknum'] = $info[0]['clicknum']+1;
			//D('article')->where('id='.$_GET['id'])->update();
			//上一篇and下一篇
			$pre = M('Document')->field('id,title')->where('id<'.$_GET['id'].' and type='.$info[0]['type'])->order('id desc')->limit('1')->select();
			$next = M('Document')->field('id,title')->where('id>'.$_GET['id'].' and type='.$info[0]['type'])->order('id desc')->limit('1')->select();
			$this->assign('pre',$pre);
...
//			ob_start();
			echo '333333';
			$this->display(T('Home@default:Article:test'));
//			$res = $this->fetch('Home@Article/test');
//			echo $res;return;
...
//			echo $_cache;return;
			...
			//跳转来继续生成
			if($_GET['id']){
				$urlList = M('Document')->where(' id>'.$_GET['id'])->order(' id asc')->limit('1')->select();
				//P($urlList); 
				//判断是否结束
				if(empty($urlList)){
					...
					echo 0000;
//					$this->redirect("/Article/htmlForArt/id/".$urlList[0]['id'],'',0.1); 
				} 
			}
		}
	}
	 ...
	 * cid 分类ID
	**/
	public function getCatepath($cid){
		$info = M('Category')->where("id=$cid")->field('id,pid,name')->find();//分类标识
		$res = '';
		if($info['pid']>0){
			$res = $this->getCatepath($info['pid']);
		}
		$res = $res.'/'.$info['name'];
		return $res;		
	}

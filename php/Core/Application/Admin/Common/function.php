 ...
 * @id int 节点ID
***/
function default_rule($id){
	$v = M('CaijiConfig')->where(array('name'=>'DEFAULT_RULE'))->getField('value');
	if($id == $v){
		$info = '<font color="#f00">默认节点</font>';
	}else{
		$url = U('Caiji/useRule?id='.$id);
		$info = '<a class="confirm ajax-get" href="'.$url.'">使用节点</a>';
	}
	return $info;
}
...
function check_caiji_config($s,$v){
	$arr = explode(',',$s);
	if(in_array($v,$arr)){
		return true;
	}else{
